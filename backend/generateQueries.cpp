...
int main() {
    ...
    ctsql_out+=std::string("CREATE TABLE products(product_id SERIAL PRIMARY KEY, product_name varchar(50) NOT NULL, product_description varchar(1000) NOT NULL, category varchar(20) NOT NULL, amount_in_stock integer NOT NULL, acme_rating float NOT NULL, user_rating float);\n")+
               ...
               std::string("CREATE TABLE proposal_comments(comment_id SERIAL PRIMARY KEY, comment_author varchar(50) NOT NULL, comment_text varchar(1500) NOT NULL);\n")+
               std::string("CREATE TABLE proposal_comment_join(proposal_id INTEGER NOT NULL, comment_id INTEGER NOT NULL);\n")+
               std::string("CREATE TABLE user_list(user_id SERIAL PRIMARY KEY, user_name varchar(50) NOT NULL, salt varchar(16) NOT NULL, hash varchar(50) NOT NULL, admin BOOLEAN NOT NULL DEFAULT false);\n");
    ctsql<<ctsql_out;
